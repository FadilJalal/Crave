...
export default function Subscriptions() {
  ...
  const fetch = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setStats({ mrr: res.data.mrr, activeCount: res.data.activeCount, trialCount: res.data.trialCount, expiredCount: res.data.expiredCount, expiringSoon: res.data.expiringSoon });
      }
  ...
  const checkExpired = async () => {
    try {
      await api.post("/api/subscription/check-expired");
      fetch();
    } catch {}
  };

  useEffect(() => { fetch(); checkExpired(); }, []);
  ...
  const handleAssign = async (e) => {
    ...
    try {
      ...
      if (res.data.success) { toast.success(res.data.message); setSelected(null); fetch(); }
  ...
  const handleCancel = async (id, name) => {
    ...
    try {
      ...
      if (res.data.success) { toast.success("Subscription cancelled"); fetch(); }
    } catch { toast.error("Error cancelling"); }
  };

  const filtered = filter === "all" ? data : data.filter(r => r.status === filter || (filter === "expiring" && r.expiringSoon));
  ...
  return (
    <div style={{ maxWidth: 1000 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
        ...
        <button onClick={fetch} style={{ padding: "8px 16px", borderRadius: 10, border: "1px solid var(--border)", background: "white", cursor: "pointer", fontSize: 13, fontWeight: 700, fontFamily: "inherit" }}>
          ↻ Refresh
        </button>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          return (
            <div key={r._id} style={{ background: "white", border: "1px solid var(--border)", borderRadius: 16, padding: "14px 18px", display: "flex", alignItems: "center", gap: 14, boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                {r.status === "active" && (
                  <button onClick={() => handleCancel(r._id, r.name)} style={{ padding: "7px 12px", borderRadius: 8, border: "none", background: "#fef2f2", color: "#dc2626", cursor: "pointer", fontSize: 12, fontWeight: 700, fontFamily: "inherit" }}>
                    Cancel
                  </button>
                )}
              </div>
        ...
        {!loading && filtered.length === 0 && <div style={{ textAlign: "center", padding: "40px", color: "var(--muted)", background: "white", border: "1px solid var(--border)", borderRadius: 14 }}>No restaurants match this filter.</div>}
      </div>
            ...
            <form onSubmit={handleAssign}>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <select style={inp} value={form.months} onChange={e => setForm(p => ({ ...p, months: e.target.value }))}>
                  {[1,2,3,6,12].map(m => <option key={m} value={m}>{m} month{m > 1 ? "s" : ""} — AED {(PLANS[form.plan]?.price || 0) * m}</option>)}
                </select>
              </div>
